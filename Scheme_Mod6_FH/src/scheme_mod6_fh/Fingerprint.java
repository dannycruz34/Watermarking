...
public class Fingerprint {
    ...
    public int insercionHuella(Parametros p) throws SQLException{
        ...
        double[] desviaciones = new double[numAttr];
        double desviacionTotal = 0.0;
        double atributosTotal = 0.0;
        for (int i = 0; i < numAttr; i++){
            desviaciones[i] = estadistica[i][1];
            desviacionTotal += estadistica[i][1];
            atributosTotal += i+1;
        }
               ...
               // System.out.println("NUM BITS A MARCAR: " + nBits);
                ...
                for (int j = numAttr-1; j >= 0; j--) {
                        ...
                        if( valorBinary.length() <= 6 ){
                            ...
                            //CALCULAR NUMERO DE BITS A INSERTAR EN EL VALOR
                            int bMark = valorBinary.length() - 5;
                            
                            //INSERCION DE BITS DE HUELLA                            
                            for (int i = 1; i <= bMark; i++) {
